    ...
    <div>
      ...
      <section className="py-24 bg-white">
        <div ref={services.ref} className="max-w-7xl mx-auto px-5 sm:px-8">
          <div className={`text-center mb-16 transition-all duration-700 ${services.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <p className="text-blue-600 text-sm font-semibold tracking-widest uppercase mb-3">Nase storitve</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div
                key={service.page}
                className={`group relative bg-white rounded-2xl overflow-hidden border border-gray-100 hover:border-gray-200 hover:shadow-xl transition-all duration-500 cursor-pointer ${services.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                style={{ transitionDelay: services.isVisible ? `${(i + 1) * 150}ms` : '0ms' }}
      ...
      <section className="py-24 bg-slate-950 relative overflow-hidden">
        ...
        <div ref={rental.ref} className="relative max-w-7xl mx-auto px-5 sm:px-8">
          <div className={`text-center mb-14 transition-all duration-700 ${rental.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <span className="inline-block bg-blue-600/20 text-blue-300 text-xs font-bold px-4 py-1.5 rounded-full uppercase tracking-widest mb-4">
              Novo v ponudbi
            </span>
          ...
          <div className={`grid grid-cols-1 lg:grid-cols-2 gap-8 items-start transition-all duration-700 delay-200 ${rental.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            ...
            <div className="bg-white/5 rounded-2xl border border-white/10 overflow-hidden">
              <div className="px-6 py-4 border-b border-white/10">
                <h4 className="text-white font-semibold">Cenik najema <span className="text-slate-400 font-normal text-sm">/ kos / mesec + DDV</span></h4>
              </div>
              <div className="divide-y divide-white/5">
                {[
                  ...
                  <div key={i} className={`flex items-center justify-between px-6 py-3.5 ${row.best ? 'bg-blue-600/10' : 'hover:bg-white/5'} transition-colors`}>
                    <div className="flex items-center gap-3">
                      {row.best && <span className="bg-blue-600 text-white text-xs font-bold px-2 py-0.5 rounded-full">Najboljsa</span>}
                      <span className="text-slate-300 text-sm">{row.period}</span>
                    </div>
                    <div>
                      <span className={`font-bold text-lg ${row.best ? 'text-blue-400' : 'text-white'}`}>{row.price} &euro;</span>
                      <span className="text-slate-500 text-sm ml-1">+ DDV</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className={`text-center mt-10 transition-all duration-700 delay-400 ${rental.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <button
              onClick={() => onPageChange('storage')}
              className="group inline-flex items-center gap-2 bg-blue-600 text-white px-7 py-3.5 rounded-full font-semibold hover:bg-blue-500 transition-all duration-300"
            >
              Vec o najemu
      ...
      <section className="py-24 bg-white">
        <div ref={cta.ref} className="max-w-5xl mx-auto px-5 sm:px-8">
          <div className={`bg-slate-950 rounded-3xl overflow-hidden transition-all duration-700 ${cta.isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
            <div className="grid grid-cols-1 lg:grid-cols-5">
              <div className="lg:col-span-3 p-10 md:p-14">
                ...
                <button
                  ...
                  className="group bg-white text-slate-900 px-7 py-3.5 rounded-full font-semibold hover:bg-blue-50 transition-all duration-300 inline-flex items-center gap-2"
                >
                  Brezplacna ponudba
                  <ArrowRight className="h-4 w-4 group-hover:translate-x-1 transition-transform" />
                </button>
